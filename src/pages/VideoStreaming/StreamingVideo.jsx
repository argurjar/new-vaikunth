...
function StreamingVideo() {
  ...
  }, [msg, status, isError, currentPage,totalvideos,dispatch,message,navigate]);
  ...
  return (
    <div className="container ">
      <div className="d-flex justify-content-between border-bottom pb-3 text-light px-5 mt-5">
        <button className=" px-4 py-1  bg-dark text-light fs-5 rounded-pill" disabled>
          Streaming Videos <BsFillArrowDownCircleFill className="ms-2  fs-3" />
        </button>
        <NavLink to={"/AddStreamingVideo"}>
          <button className="btn btn-dark text-light btn-add">Add Video</button>
        </NavLink>
      </div>
        ...
        <colgroup>
          ...
          <tbody>
            {displayedVideos?.map((item) => {
              const textColor = item.status === "Live" ? "#fca311" : "red";
              return (
                <tr key={item._id}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => navigate(`/EditStreamingVideo/${item._id}`)}
                    >
                    ...
                    <button
                      type="button"
                      className="btn  ms-1 btn-show"
                      onClick={() => navigate(`/ShowStreamingVideo/${item._id}`)}
                    >
